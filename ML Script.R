...
taxlist <- taxlist[taxlist$Nama.Vendor=="Huawei Tech Investment",]
set.seed(123)
...
c50_model <- C5.0(x=train_dtm, y=train_class)
ripper_model <- JRip(train_class~., data = as.data.frame(train_dtm))

summary(taxlist_model)
